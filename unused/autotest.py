...
class TestStatus(Enum):
  ...
  def to_msg(self):
    ...
    return {
        ...
        TestStatus.OUTPUT_NOT_FOUND: '\033[0;31mOUTPUT NO FOUND\033[0m',
...
def decode_result(result: 'subprocess.CompletedProcess[str]') -> Tuple[str, str]:
  '''
...
def execute(cmd: str, timeout: int, error_status: TestStatus,
  ...
  try:
    if pipe_cmd:
      ...
      if pipe.wait():
        print("pipe wait")
        return TestResult(error_status, output_error=(decode_bytes(pipe.stdout.read()),
...
def build_make_project(repo_dir: str, working_dir: str) -> Tuple[str, str]:
  ...
  build_cmd = f'make BUILD_DIR="{working_dir}" {flags} -C "{repo_dir}"'
...
def run_output(mode: TestMode, case: TestCase, exe: str) -> TestResult:
  ...
  print("return code", f'{result.returncode}')
...
if __name__ == '__main__':
  ...
  if not cases:
    eprint(f'no test cases found in dierctory "{test_case_dir}"')
    sys.exit(-1)
