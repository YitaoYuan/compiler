...
void ArithmeticAST::dump() {
	...
	unordered_map<int, tmpid>cp[2] = {s2t[0], s2t[1]}; // back up
	int used_id_cp = used_id, label_id_cp = label_id;
	// 幸好我程序用到的全局变量不多，不然得GG
	...
	if(!is_compile_known_const && is_short_circuit && 
	    (y->treesize > short_circuit_threshold || y->has_side_effect) ) {
		// y开销大或者调用了函数，做短路运算 


		s2t[0] = cp[0], s2t[1] = cp[1]; // recover
		used_id = used_id_cp, label_id = label_id_cp;
